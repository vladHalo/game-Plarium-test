...
namespace ShootStage.Scripts
{
    public class GameViewSizeHelper
    {
        ...
        public static void AddCustomSize(GameViewSizeGroupType groupType, GameViewSizeType type, int width, int height,
            string baseText)
        {
            AddCustomSize(groupType,
                new GameViewSize { type = type, width = width, height = height, baseText = baseText });
        }

        public static bool RemoveCustomSize(GameViewSizeGroupType groupType, GameViewSizeType type, int width,
            int height,
            string baseText)
        {
        ...
        public static bool Contains(GameViewSizeGroupType groupType, GameViewSizeType type, int width, int height,
            string baseText)
        {
            _gameViewSize = new GameViewSize { type = type, width = width, height = height, baseText = baseText };
        ...
        public static void ChangeGameViewSize(GameViewSizeGroupType groupType, GameViewSizeType type, int width,
            int height,
            string baseText)
        {
            ChangeGameViewSize(groupType,
                new GameViewSize { type = type, width = width, height = height, baseText = baseText });
        }
        ...
        public static void ChangeGameViewSize(GameViewSizeGroupType groupType, GameViewSize gameViewSize)
        {
            ...
            {
                Debug.LogError(string.Format("GameViewSizeGroupType is {0}. but Current GameViewSizeGroupType is {1}.",
            ...
            if (index != -1)
            {
                PropertyInfo selectedSizeIndex =
                    Types.gameView.GetProperty("selectedSizeIndex", BindingFlags.Instance | BindingFlags.NonPublic);
        ...
        {
            EditorWindow gameView = EditorWindow.GetWindow(Types.gameView);
            PropertyInfo currentSizeGroupType = Types.gameView.GetProperty("currentSizeGroupType",
        ...
        private static bool GameViewSize_Equals(GameViewSize a, object b)
        {
            ...
            GameViewSizeType b_sizeType =
                (GameViewSizeType)Enum.Parse(typeof(GameViewSizeType), GetGameSizeProperty(b, "sizeType").ToString());
        ...
        static object GetGroup(GameViewSizeGroupType groupType, object instance_gameViewSizes)
        {
            ...
            return instance_gameViewSizes.GetType().GetMethod("GetGroup",
                    ...
                    returnTypes,
                    null)
                ?.Invoke(instance_gameViewSizes, parameters);
        }
        ...
        static object GetGameViewSize(object instance_gameViewSizeGroup, int i)
        {
            ...
            return instance_gameViewSizeGroup.GetType().GetMethod("GetGameViewSize",
                    ...
                    returnTypes,
                    null)
                ?.Invoke(instance_gameViewSizeGroup, parameters);
        }
        ...
        static int GetCustomCount(object instance_gameViewSizeGroup)
        {
            return (int)instance_gameViewSizeGroup.GetType().GetMethod("GetCustomCount",
                    ...
                    new Type[0],
                    null)
                ?.Invoke(instance_gameViewSizeGroup, Array.Empty<object>())!;
        }
        ...
        private static class Types
        {
            ...
            public static Type gameViewSizes = assembly.GetType("UnityEditor.ScriptableSingleton`1")
                .MakeGenericType(assembly.GetType("UnityEditor.GameViewSizes"));
        }
